...
function doInitAll() {
	...
	$plus.click(function() {
		...
		//修改数据库购物车中的商品数量
		var url = "/cart/doChangeProdAmount";
		var params = {
			"userId" : 1,
			"prodId" : parseInt($(this).parents('.order_lists').find('.son_check').val()),
			"amount" : $count
		};
		console.log(params);
		$.getJSON(url, params);
	...
	$('.dialog-sure').click(
			function() {
				//从数据库删除购物车中的商品
				var url = "/cart/doDeleteProd";
				var params = {
					"userId" : 1,
					"prodId" : parseInt($order_lists.find('.son_check').val())
				};
				console.log(params);
				$.getJSON(url, params);
				$order_lists.remove();
